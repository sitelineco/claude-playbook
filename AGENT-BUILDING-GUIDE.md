...
- [ ] **Deployment mapped & "live" verified:** confirmed the exact owner/repo/branch production builds from, every separate deploy target (frontend, edge functions, migrations), and that fixes were verified on the *running* system — not just committed (Part 11.5)

---

## Part 11.5 — Deployment reality: map it before you build, verify fixes are LIVE

> Driven by a real incident (see `POST-MORTEM-salt-deploy-drift.md`): a full session of fixes —
> including live cross-tenant security holes — was committed to a repo that **doesn't deploy**, while
> production built from a *different repo with the same name under a different owner*. Nothing was
> live, but everything *looked* done.

Understanding the *codebase* is not enough. A fix that's committed but not deployed — or committed to
a repo that doesn't deploy — is worse than no fix: it looks done. **Map how this project actually
reaches production, and treat "live & verified" (not "committed") as the definition of done.**

### Map the deployment (do this in Phase 1, with EVIDENCE — not handoff docs)
1. **Host:** `curl -I https://<domain>` → `server: Vercel` / `x-nf-*` Netlify / etc.
2. **Exact owner/repo/branch it builds from:** confirm in the host dashboard (e.g. Vercel → Settings
   → Git). **Watch for same-named repos under different owners** (`sitelineco/app` vs
   `SaltHomeStudio/app` were indistinguishable and only one deployed). Can't confirm? Prove it: a
   string in the deployed bundle that's in the repo's git history ⇒ builds from it; absent from all
   history ⇒ it does not.
3. **Separate deploy targets:** the frontend host is usually NOT the whole picture —
   - **Edge functions** (Supabase, etc.) deploy on their own track (`supabase functions deploy`,
     often a separate CI job + secret like `SUPABASE_ACCESS_TOKEN`). A green frontend deploy says
     nothing about them.
   - **DB migrations** likewise.
4. **Secrets/quirks that gate deploys** (e.g. Vercel Hobby only deploys commits authored by a
   specific account; required CI secrets).

Record all of this in the project's `CLAUDE.md` under a **"Deployment"** heading so no future
session re-learns it the hard way.

### "Committed" ≠ "live"
- After any fix, **verify on the deployed system**, not the diff: probe the live URL/API, re-fetch
  the bundle, hit the endpoint. (Safe probe: an unauthenticated call to a now-protected function
  should return `401` *before* any side effect.)
- If repo and production have **drifted** (deployed code not in the repo), **STOP and reconcile the
  deploy source first** — fixes to a non-deploying repo can't ship.
- Converge on **one** canonical repo the host builds from; archive the rest.

### Make the `frontend-auditor` own drift (add to its remit + run in CI)
> "Detect repo-vs-deployed drift: fetch the production bundle and confirm it corresponds to this
> repo's HEAD (spot-check a known string/commit); confirm the host's connected owner/repo/branch;
> flag any deployed behavior with no source in this repo."

### Two concrete gotchas this incident surfaced
- **Image-optimized URLs break filename de-dup** (galleries): de-dupe/compare on the **raw
  identifier** (storage object `name`), never a transformed/optimized URL — optimizers (Supabase
  `render/image`, Vercel/Next image, CDNs) rewrite the path so every URL's last segment collapses to
  the same value and silently matches-and-filters everything. Apply the transform only for *display*.
- **Don't trust stored mimetypes for user uploads** (Salt's JPGs were stored as `text/plain`): gate
  image filtering on the **filename extension** (`/\.(jpe?g|png|gif|webp|avif|heic|tiff?)$/i`), not
  `metadata.mimetype`.
